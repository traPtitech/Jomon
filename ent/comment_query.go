...
func (cq *CommentQuery) QueryRequest() *RequestQuery {
	query := &RequestQuery{config: cq.config}
...
func (cq *CommentQuery) WithRequest(opts ...func(*RequestQuery)) *CommentQuery {
	query := &RequestQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withRequest = query
	return cq
}
...
func (cq *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if query := cq.withRequest; query != nil {
		if err := cq.loadRequest(ctx, query, nodes, nil,
			func(n *Comment, e *Request) { n.Edges.Request = e }); err != nil {
			return nil, err
		}
	}
...
func (cq *CommentQuery) loadRequest(ctx context.Context, query *RequestQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Request)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "request_comment" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
