...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withTransaction; query != nil {
		if err := tq.loadTransaction(ctx, query, nodes,
...
func (tq *TagQuery) loadTransaction(ctx context.Context, query *TransactionQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Transaction)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*Tag)
	nids := make(map[uuid.UUID]map[*Tag]struct{})
	for i, node := range nodes {
