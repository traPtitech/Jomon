...
func (gq *GroupQuery) QueryGroupBudget() *GroupBudgetQuery {
	query := (&GroupBudgetClient{config: gq.config}).Query()
...
func (gq *GroupQuery) QueryOwner() *UserQuery {
	query := (&UserClient{config: gq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (gq *GroupQuery) WithGroupBudget(opts ...func(*GroupBudgetQuery)) *GroupQuery {
	query := (&GroupBudgetClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gq.withGroupBudget = query
	return gq
}
...
func (gq *GroupQuery) WithOwner(opts ...func(*UserQuery)) *GroupQuery {
	query := (&UserClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (gq *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := gq.withGroupBudget; query != nil {
		if err := gq.loadGroupBudget(ctx, query, nodes,
			func(n *Group) { n.Edges.GroupBudget = []*GroupBudget{} },
			func(n *Group, e *GroupBudget) { n.Edges.GroupBudget = append(n.Edges.GroupBudget, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := gq.withOwner; query != nil {
		if err := gq.loadOwner(ctx, query, nodes,
			func(n *Group) { n.Edges.Owner = []*User{} },
			func(n *Group, e *User) { n.Edges.Owner = append(n.Edges.Owner, e) }); err != nil {
			return nil, err
		}
	}
...
func (gq *GroupQuery) loadGroupBudget(ctx context.Context, query *GroupBudgetQuery, nodes []*Group, init func(*Group), assign func(*Group, *GroupBudget)) error {
	...
	for _, n := range neighbors {
		...
		if fk == nil {
			return fmt.Errorf(`foreign-key "group_group_budget" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (gq *GroupQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Group, init func(*Group), assign func(*Group, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(group.UserTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(group.UserPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(group.UserPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(group.UserPrimaryKey[0]))
...
func (gq *GroupQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Group, init func(*Group), assign func(*Group, *User)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*Group)
	nids := make(map[uuid.UUID]map[*Group]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(group.OwnerTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(group.OwnerPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(group.OwnerPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(group.OwnerPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
