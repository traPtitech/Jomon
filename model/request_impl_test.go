...
func TestEntRepository_GetRequests(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		if assert.Len(t, got, 2) && got[0].ID == request1.ID {
...
func TestEntRepository_CreateRequest(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		request1, err := repo.CreateRequest(ctx, amount, title, content, []*Tag{tag}, group, user.ID)
		assert.NoError(t, err)
...
func TestEntREpository_UpdateRequest(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		amount := 100
		updatedRequest, err := repo.UpdateRequest(ctx, request.ID, amount, request.Title, request.Content, []*Tag{tag1}, group)
		assert.NoError(t, err)
		...
		assert.Equal(t, updatedRequest.Tags[0].ID, request.Tags[0].ID)
		assert.Equal(t, updatedRequest.Tags[0].Name, request.Tags[0].Name)
		assert.Equal(t, updatedRequest.Tags[0].Description, request.Tags[0].Description)
