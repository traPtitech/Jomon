...
func TestEntRepository_GetRequests(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		request1, err := repo.CreateRequest(ctx, random.Numeric(t, 1000000), random.AlphaNumeric(t, 40), []*Tag{tag}, group, user1.ID)
		require.NoError(t, err)
		time.Sleep(1 * time.Second)
		request2, err := repo.CreateRequest(ctx, random.Numeric(t, 1000000), random.AlphaNumeric(t, 40), []*Tag{tag}, group, user2.ID)
		require.NoError(t, err)
...
func TestEntRepository_CreateRequest(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		request, err := repo.CreateRequest(ctx, amount, title, []*Tag{tag}, group, user.ID)
		assert.NoError(t, err)
	...
	t.Run("UnknownUser", func(t *testing.T) {
		...
		_, err = repo.CreateRequest(ctx, amount, title, []*Tag{tag}, group, uuid.New())
		assert.Error(t, err)
	})
	...
	t.Run("UnknownTag", func(t *testing.T) {
		...
		_, err = repo.CreateRequest(ctx, amount, title, []*Tag{tag}, group, user.ID)
		assert.Error(t, err)
	})
...
func TestEntRepository_GetRequest(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		group, err := repo.CreateGroup(ctx, random.AlphaNumeric(t, 20), random.AlphaNumeric(t, 30), &budget, &[]User{*owner})
		require.NoError(t, err)
		request, err := repo.CreateRequest(ctx, random.Numeric(t, 1000000), random.AlphaNumeric(t, 40), []*Tag{tag}, group, user.ID)
		require.NoError(t, err)
...
func TestEntREpository_UpdateRequest(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		amount := random.Numeric(t, 100000)
		updatedRequest, err := repo.UpdateRequest(ctx, request.ID, amount, request.Title, []*Tag{tag}, group)
		assert.NoError(t, err)
	...
	t.Run("Success2", func(t *testing.T) {
		...
		title := random.AlphaNumeric(t, 40)
		updatedRequest, err := repo.UpdateRequest(ctx, request.ID, request.Amount, title, []*Tag{tag}, group)
		assert.NoError(t, err)
		assert.Equal(t, updatedRequest.ID, request.ID)
		assert.Equal(t, updatedRequest.Status, request.Status)
	...
	t.Run("UnknownTag", func(t *testing.T) {
		...
		_, err = repo.UpdateRequest(ctx, request.ID, request.Amount, request.Title, []*Tag{unknownTag}, group)
		assert.Error(t, err)
	})
	...
	t.Run("UnknownGroup", func(t *testing.T) {
		...
		_, err = repo.UpdateRequest(ctx, request.ID, request.Amount, request.Title, []*Tag{tag}, unknownGroup)
		assert.Error(t, err)
	})
}
