...
// UserSubjectMutation represents an operation that mutates the UserSubject nodes in the graph.
type UserSubjectMutation struct {
	config
	op            Op
	typ           string
	id            *string
	...
	oldValue      func(context.Context) (*UserSubject, error)
	predicates    []predicate.UserSubject
}

var _ ent.Mutation = (*UserSubjectMutation)(nil)

// usersubjectOption allows management of the mutation configuration using functional options.
type usersubjectOption func(*UserSubjectMutation)

// newUserSubjectMutation creates new mutation for the UserSubject entity.
func newUserSubjectMutation(c config, op Op, opts ...usersubjectOption) *UserSubjectMutation {
	m := &UserSubjectMutation{
		config:        c,
		op:            op,
		typ:           TypeUserSubject,
		clearedFields: make(map[string]struct{}),
...
// withUserSubjectID sets the ID field of the mutation.
func withUserSubjectID(id string) usersubjectOption {
	return func(m *UserSubjectMutation) {
		var (
			err   error
			once  sync.Once
			value *UserSubject
		)
		m.oldValue = func(ctx context.Context) (*UserSubject, error) {
			once.Do(func() {
				if m.done {
					err = errors.New("querying old values post mutation is not allowed")
				} else {
					value, err = m.Client().UserSubject.Get(ctx, id)
				}
			})
			return value, err
		}
...
// withUserSubject sets the old UserSubject of the mutation.
func withUserSubject(node *UserSubject) usersubjectOption {
	return func(m *UserSubjectMutation) {
		m.oldValue = func(context.Context) (*UserSubject, error) {
			return node, nil
		}
		m.id = &node.ID
	}
...
// Client returns a new `ent.Client` from the mutation. If the mutation was
// executed in a transaction (ent.Tx), a transactional client is returned.
func (m UserSubjectMutation) Client() *Client {
	client := &Client{config: m.config}
	client.init()
	return client
}
...
// Tx returns an `ent.Tx` for mutations that were executed in transactions;
// it returns an error otherwise.
func (m UserSubjectMutation) Tx() (*Tx, error) {
	if _, ok := m.driver.(*txDriver); !ok {
		return nil, errors.New("ent: mutation is not running in a transaction")
	}
	tx := &Tx{config: m.config}
...
// SetID sets the value of the id field. Note that this
// operation is only accepted on creation of UserSubject entities.
func (m *UserSubjectMutation) SetID(id string) {
	m.id = &id
}

// ID returns the ID value in the mutation. Note that the ID is only available
// if it was provided to the builder or after it was returned from the database.
func (m *UserSubjectMutation) ID() (id string, exists bool) {
	if m.id == nil {
		return
	}
	return *m.id, true
...
// as sql.LevelSerializable, the returned ids match the ids of the rows that will be updated
// or updated by the mutation.
func (m *UserSubjectMutation) IDs(ctx context.Context) ([]string, error) {
	switch {
	case m.op.Is(OpUpdateOne | OpDeleteOne):
		id, exists := m.ID()
		if exists {
		...
		fallthrough
	case m.op.Is(OpUpdate | OpDelete):
		return m.Client().UserSubject.Query().Where(m.predicates...).IDs(ctx)
	default:
		return nil, fmt.Errorf("IDs is not allowed on %s operations", m.op)
	}
}
...
func (m *UserSubjectMutation) SetUserID(u uuid.UUID) {
	m.user = &u
}

// UserID returns the value of the "user_id" field in the mutation.
func (m *UserSubjectMutation) UserID() (r uuid.UUID, exists bool) {
	v := m.user
	if v == nil {
		return
	}
...
// OldUserID returns the old "user_id" field's value of the UserSubject entity.
// If the UserSubject object wasn't provided to the builder, the object is fetched from the database.
// An error is returned if the mutation operation is not UpdateOne, or the database query fails.
func (m *UserSubjectMutation) OldUserID(ctx context.Context) (v uuid.UUID, err error) {
	if !m.op.Is(OpUpdateOne) {
...
func (m *UserSubjectMutation) ResetUserID() {
	m.user = nil
}

// ClearUser clears the "user" edge to the User entity.
func (m *UserSubjectMutation) ClearUser() {
	m.cleareduser = true
	m.clearedFields[usersubject.FieldUserID] = struct{}{}
}

// UserCleared reports if the "user" edge to the User entity was cleared.
func (m *UserSubjectMutation) UserCleared() bool {
	return m.cleareduser
}
...
// Note that IDs always returns len(IDs) <= 1 for unique edges, and you should use
// UserID instead. It exists only for internal usage by the builders.
func (m *UserSubjectMutation) UserIDs() (ids []uuid.UUID) {
	if id := m.user; id != nil {
		ids = append(ids, *id)
	}
	return
...
// ResetUser resets all changes to the "user" edge.
func (m *UserSubjectMutation) ResetUser() {
	m.user = nil
	m.cleareduser = false
}

// Where appends a list predicates to the UserSubjectMutation builder.
func (m *UserSubjectMutation) Where(ps ...predicate.UserSubject) {
	m.predicates = append(m.predicates, ps...)
}

// WhereP appends storage-level predicates to the UserSubjectMutation builder. Using this method,
// users can use type-assertion to append predicates that do not depend on any generated package.
func (m *UserSubjectMutation) WhereP(ps ...func(*sql.Selector)) {
	p := make([]predicate.UserSubject, len(ps))
	for i := range ps {
		p[i] = ps[i]
	}
	m.Where(p...)
...
// Op returns the operation name.
func (m *UserSubjectMutation) Op() Op {
	return m.op
}

// SetOp allows setting the mutation operation.
func (m *UserSubjectMutation) SetOp(op Op) {
	m.op = op
}

// Type returns the node type of this mutation (UserSubject).
func (m *UserSubjectMutation) Type() string {
	return m.typ
}
...
// order to get all numeric fields that were incremented/decremented, call
// AddedFields().
func (m *UserSubjectMutation) Fields() []string {
	fields := make([]string, 0, 1)
	if m.user != nil {
		fields = append(fields, usersubject.FieldUserID)
	}
	return fields
}
...
// return value indicates that this field was not set, or was not defined in the
// schema.
func (m *UserSubjectMutation) Field(name string) (ent.Value, bool) {
	switch name {
...
// returned if the mutation operation is not UpdateOne, or the query to the
// database failed.
func (m *UserSubjectMutation) OldField(ctx context.Context, name string) (ent.Value, error) {
	switch name {
	case usersubject.FieldUserID:
		return m.OldUserID(ctx)
	}
	return nil, fmt.Errorf("unknown UserSubject field %s", name)
}

// SetField sets the value of a field with the given name. It returns an error if
// the field is not defined in the schema, or if the type mismatched the field
// type.
func (m *UserSubjectMutation) SetField(name string, value ent.Value) error {
	switch name {
	case usersubject.FieldUserID:
	...
	return fmt.Errorf("unknown UserSubject field %s", name)
}

// AddedFields returns all numeric fields that were incremented/decremented during
// this mutation.
func (m *UserSubjectMutation) AddedFields() []string {
	return nil
}
...
// with the given name. The second boolean return value indicates that this field
// was not set, or was not defined in the schema.
func (m *UserSubjectMutation) AddedField(name string) (ent.Value, bool) {
	return nil, false
}
...
// the field is not defined in the schema, or if the type mismatched the field
// type.
func (m *UserSubjectMutation) AddField(name string, value ent.Value) error {
	switch name {
	}
	return fmt.Errorf("unknown UserSubject numeric field %s", name)
}

// ClearedFields returns all nullable fields that were cleared during this
// mutation.
func (m *UserSubjectMutation) ClearedFields() []string {
	return nil
}

// FieldCleared returns a boolean indicating if a field with the given name was
// cleared in this mutation.
func (m *UserSubjectMutation) FieldCleared(name string) bool {
	_, ok := m.clearedFields[name]
	return ok
}
...
// ClearField clears the value of the field with the given name. It returns an
// error if the field is not defined in the schema.
func (m *UserSubjectMutation) ClearField(name string) error {
	return fmt.Errorf("unknown UserSubject nullable field %s", name)
}

// ResetField resets all changes in the mutation for the field with the given name.
// It returns an error if the field is not defined in the schema.
func (m *UserSubjectMutation) ResetField(name string) error {
	switch name {
	case usersubject.FieldUserID:
		m.ResetUserID()
		return nil
...
// AddedEdges returns all edge names that were set/added in this mutation.
func (m *UserSubjectMutation) AddedEdges() []string {
	edges := make([]string, 0, 1)
	if m.user != nil {
		edges = append(edges, usersubject.EdgeUser)
	}
	return edges
}
...
// AddedIDs returns all IDs (to other nodes) that were added for the given edge
// name in this mutation.
func (m *UserSubjectMutation) AddedIDs(name string) []ent.Value {
	switch name {
	case usersubject.EdgeUser:
		if id := m.user; id != nil {
			return []ent.Value{*id}
		}
	}
...
// RemovedEdges returns all edge names that were removed in this mutation.
func (m *UserSubjectMutation) RemovedEdges() []string {
	edges := make([]string, 0, 1)
	return edges
}
...
// RemovedIDs returns all IDs (to other nodes) that were removed for the edge with
// the given name in this mutation.
func (m *UserSubjectMutation) RemovedIDs(name string) []ent.Value {
	return nil
}

// ClearedEdges returns all edge names that were cleared in this mutation.
func (m *UserSubjectMutation) ClearedEdges() []string {
	edges := make([]string, 0, 1)
	if m.cleareduser {
		edges = append(edges, usersubject.EdgeUser)
	}
...
// EdgeCleared returns a boolean which indicates if the edge with the given name
// was cleared in this mutation.
func (m *UserSubjectMutation) EdgeCleared(name string) bool {
	switch name {
	case usersubject.EdgeUser:
		return m.cleareduser
	}
...
// ClearEdge clears the value of the edge with the given name. It returns an error
// if that edge is not defined in the schema.
func (m *UserSubjectMutation) ClearEdge(name string) error {
	switch name {
	case usersubject.EdgeUser:
		m.ClearUser()
		return nil
	}
	return fmt.Errorf("unknown UserSubject unique edge %s", name)
}

// ResetEdge resets all changes to the edge with the given name in this mutation.
// It returns an error if the edge is not defined in the schema.
func (m *UserSubjectMutation) ResetEdge(name string) error {
	switch name {
	case usersubject.EdgeUser:
		m.ResetUser()
		return nil
	}
	return fmt.Errorf("unknown UserSubject edge %s", name)
}
