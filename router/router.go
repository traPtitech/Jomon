...
func SetRouting(e *echo.Echo, service Service) {
	...
	apiImages := e.Group("/images", service.AuthUserMiddleware)
	{
		apiImages.GET("/:imageId", service.GetImages)
		apiImages.DELETE("/:imageId", service.DeleteImages)
	}

	apiComments := e.Group("/applications/:applicationId/comments", service.AuthUserMiddleware)
	{
	...
	apiUsers := e.Group("/users", service.AuthUserMiddleware)
	{
