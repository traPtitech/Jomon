...
func (h Handlers) CheckRequestCreatorMiddleware(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if !ok {
			return echo.NewHTTPError(http.StatusInternalServerError, "session request creator key is not set")
		}
		if creator != user.ID {
...
func (h Handlers) CheckFileCreatorMiddleware(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if !ok {
			return echo.NewHTTPError(http.StatusInternalServerError, "session file creator key is not set")
		}
		if creator != user.ID {
			return echo.NewHTTPError(http.StatusForbidden, "you are not creator")
		}
